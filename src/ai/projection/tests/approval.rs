//! Tool-approval and resolution scenarios — the `requires_approval` /
//! `resolved` / `decisions` interplay that `OpenTurn::flush_into` and
//! `mark_resolved_calls` (`src/ai/projection/turn.rs`) produce together.
